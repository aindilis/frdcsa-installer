...
inline TERM* ClauseAssembler::shareTmpLiteral(const TMP_LITERAL* lit,Clause* cl) 
{
  CALL("shareTmpLiteral(const TMP_LITERAL* lit,const Clause* cl)");
  if (lit->isEquality())
    {
...
}; // TERM* ClauseAssembler::shareTmpLiteral(const TMP_LITERAL* lit,Clause* cl)
  ...
  if (lit->arity())
      ...
      if (!(leaf->NumOfClauses())) // no more clauses with this literal
	{
	  ...
	  if (last_fork)
	    ...
	    {
	      *branch_to_destroy = (DTREE_NODE*)0;
	    }; 
      ...
      if (!((*(_litSharing.Tree(*lit)))->NumOfClauses())) // no more clauses with this literal
	{
	  ...
	  *(_litSharing.Tree(*lit)) = (DTREE_NODE*)0;
	};
    };
...
Clause* ClauseAssembler::assembleUnitClause(TMP_LITERAL* lit,OpenClauseBackground& ancestors) 
{
  CALL("assembleUnitClause(TMP_LITERAL* lit,OpenClauseBackground& ancestors)");
  ...
  if (num_of_max > 1)
    {
      ...
      _varWeighting.Reset();
      for (TMP_LITERAL* el = max_lits.first(); el; el = el->next())
	_varWeighting.AnotherTermVariables(el->variableOccurences());  
      for (TMP_LITERAL* el = nonmax_lits.first(); el; el = el->next())
	_varWeighting.AnotherTermVariables(el->variableOccurences());  
...
TERM* ClauseAssembler::assembleUnsharedLit(const TMP_LITERAL& lit) 
{
  CALL("assembleUnsharedLit(const TMP_LITERAL& lit)");
  ...
  if (header.IsEquality())
    {
      Flatterm::Iterator iter(lit.arg1());
    ...
    {
      Flatterm::Iterator iter(lit.flatterm());
      iter.NextSym();
      while (iter.NotEndOfTerm())
	{
	  _openUnsharedTerm.symbol(iter.CurrPos()->symbolRef()); 
...
void ClauseAssembler::writeIntoPagedPassiveClauseStorage(PagedPassiveClauseStorage& storage,TMP_LITERAL* lit)
{
  CALL("writeIntoPagedPassiveClauseStorage(PagedPassiveClauseStorage& storage,TMP_LITERAL* lit)");
