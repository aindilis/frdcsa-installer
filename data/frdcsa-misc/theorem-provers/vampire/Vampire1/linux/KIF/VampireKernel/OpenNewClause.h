 ...
 public:
  ...
  bool OpenNonpropLiteral(const TERM& header,bool inherentlySelected)
      ...
      if (_collectingNonanswerLiteral)
	{
      ...
      if (!_newLiteral) _newLiteral = TMP_LITERAL::freshLiteral();
      ASSERT(_newLiteral);
      _newLiteral->reset();
  ...
  bool CloseEqLiteral()
      ...
      if (_newLiteral->argsAreEqual())
	{
      ...
      for (TMP_LITERAL* el = _allLiterals.first();
	   el;
	   el = el->next())
	{
	  if (el->isEquality())
  ...
  bool PushPropLit(const TERM& header,bool inherentlySelected)
      ...
      if (header == TERM::builtInFalse())
	{
      ...
      for (const TMP_LITERAL* el = _allLiterals.first();
	   el;
	   el = el->next())
	{
