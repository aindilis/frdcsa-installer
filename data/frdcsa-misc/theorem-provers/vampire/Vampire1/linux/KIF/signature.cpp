...
Signature::Symbol* Signature::Iterator::next ()
{
  ...
  ASS( debug_signature, _currentList->isNonEmpty() );
...
Atom* Signature::createAnswerAtom (void* args)
{
  Term::List* ts = reinterpret_cast <Term::List*> (args);
  _answer = createPred ( "$answer", ts->length ());
  return new Atom (_answer, ts);
