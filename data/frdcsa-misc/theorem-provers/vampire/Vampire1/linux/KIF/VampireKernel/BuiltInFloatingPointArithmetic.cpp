...
BuiltInFloatingPointArithmetic::BuiltInFloatingPointArithmetic(const char* name,
  ...
  _fact((Clause*)0),
#ifdef NO_DEBUG
  _symbolInterpretation((NativeSymbolDescriptor*)0,(int)0)
#else
  _symbolInterpretation((NativeSymbolDescriptor*)0,"BuiltInFloatingPointArithmetic::_symbolInterpretation")
#endif
{
  ...
  _binaryPlusDesc.setInterpretation((void*)binaryPlusInterpretation);

  _unaryPlusDesc.setNativeName("unary_plus");  
  _unaryPlusDesc.setArity(1UL);
  _unaryPlusDesc.setInterpretation((void*)unaryPlusInterpretation);

  _binaryMinusDesc.setNativeName("binary_minus");
  _binaryMinusDesc.setArity(2UL);
  _binaryMinusDesc.setInterpretation((void*)binaryMinusInterpretation);

  _unaryMinusDesc.setNativeName("unary_minus");
  _unaryMinusDesc.setArity(1UL);
  _unaryMinusDesc.setInterpretation((void*)unaryMinusInterpretation);

  _multiplyDesc.setNativeName("multiply");
  _multiplyDesc.setArity(2UL);
  _multiplyDesc.setInterpretation((void*)multiplyInterpretation);

  _divideDesc.setNativeName("divide"); 
  _divideDesc.setArity(2UL); 
  _divideDesc.setInterpretation((void*)divideInterpretation);

  _min2Desc.setNativeName("min2"); 
  _min2Desc.setArity(2UL); 
  _min2Desc.setInterpretation((void*)min2Interpretation);


  _max2Desc.setNativeName("max2"); 
  _max2Desc.setArity(2UL); 
  _max2Desc.setInterpretation((void*)max2Interpretation);



  _greaterDesc.setNativeName("greater"); 
  _greaterDesc.setArity(2UL); 
  _greaterDesc.setInterpretation((void*)greaterInterpretation);

  _greaterOrEqualDesc.setNativeName("greater_or_equal"); 
  _greaterOrEqualDesc.setArity(2UL); 
  _greaterOrEqualDesc.setInterpretation((void*)greaterOrEqualInterpretation);

  _lessDesc.setNativeName("less"); 
  _lessDesc.setArity(2UL); 
  _lessDesc.setInterpretation((void*)lessInterpretation);

  _lessOrEqualDesc.setNativeName("less_or_equal"); 
  _lessOrEqualDesc.setArity(2UL); 
  _lessOrEqualDesc.setInterpretation((void*)lessOrEqualInterpretation);
...
bool BuiltInFloatingPointArithmetic::interpret(ulong inputSymbolId,
...
}; // bool BuiltInFloatingPointArithmetic::interpret(ulong inputSymbolId,const char* nativeSymbol)
...
BuiltInFloatingPointArithmetic::interpretation(ulong symbolId)
{
  ...
  return (NativeSymbolDescriptor*)0;
...
bool BuiltInFloatingPointArithmetic::evaluateTerm(const Flatterm* term,TMP_LITERAL* newLit)
{
  CALL("evaluateTerm(const Flatterm* term,TMP_LITERAL* newLit)");
...
Clause* BuiltInFloatingPointArithmetic::simplify(TMP_LITERAL* lit,TMP_LITERAL* newLit)
{
  ...
  if (canBeEvaluated(lit->flatterm()))
    {
      bool atomValue;
      if (!evaluateAtom(lit->flatterm(),atomValue)) return (Clause*)0;
      bool literalValue;
      if (lit->isPositive()) 
	{
	...
	literalValue = !atomValue;
      ...
      if (!_fact)
	{
	  _fact = new Clause((LiteralList*)0);
