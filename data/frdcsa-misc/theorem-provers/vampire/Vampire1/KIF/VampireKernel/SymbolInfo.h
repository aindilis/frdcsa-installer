...
class SymbolInfo
{
 public: 
  ...
  const Clause* commutativityLaw(ulong funNum) const
  {
   ...
   if (funNum >= _table.size()) return (const Clause*)0;
  ...
  const Clause* symmetryLaw(ulong predNum) const
  {
   ...
   if (predNum >= _table.size()) return (const Clause*)0;
  ...
  class Entry
  {
   public:
    ...
    void init() 
    {
     ...
     commutativityLaw = (const Clause*)0; 
     symmetryLaw = (const Clause*)0; 
    };  
   public:
 ...
 private:
  ...
  Array<GlobAlloc,Entry,128UL,SymbolInfo> _table;
