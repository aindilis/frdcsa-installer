  ...
  if (DestructionMode::isThorough())
    {
      ...
      optDescs = (OptDescList*)0;
      StrList::destroyList(nonoptMidValues);
      nonoptMidValues = (StrList*)0;
    };
};
...
bool CommandLine::checkType(const char* str,ParamType tp,const Enum* possibleValues)
{
 ...
 bool dummyBool;
  ...
  switch(tp)
  { 
    case Long: return Values::readInf(str,dummyBool) || StrToX::conv(dummyLong,str);
    case Double: return Values::readInf(str,dummyBool) || StrToX::conv(dummyDouble,str); 
    case String:
      if (possibleValues)
	{
 ...
 if (bool(*this))
  {  
   ...
   if (readInf(StrListIter::currEl(),posInf))
    ...
    StrToX str(StrListIter::currEl());
   #else
    StrToX str((char*)StrListIter::currEl());   
   #endif
   StrListIter::next();
   str.read(n);
   return true;
...
CommandLine::Values::operator bool() const { return StrListIter::rest(); };
