 ...
 class EquivalenceRelation
  {
    ...
    class Iterator // to enumerate a minimal frame of an equivalence relation
     {
      public:
       Iterator() {};
       Iterator(const EquivalenceRelation& EquivalenceRelation) 
        : next(EquivalenceRelation.registered), 
          end(EquivalenceRelation.next_registered),
          can_rep(EquivalenceRelation.can_rep) 
        {};
       void Reset(const EquivalenceRelation& EquivalenceRelation) 
        {
         next = EquivalenceRelation.registered;
         end = EquivalenceRelation.next_registered;
         can_rep = EquivalenceRelation.can_rep;
    ...
    class EqClass // representation of an equivalence class
     {
      public:
       EqClass() {};
       EqClass(EquivalenceRelation& rel,const ulong& x) { init(rel,x); }; 
   ...
   public: 
    ...
    ~EquivalenceRelation() { CALLM(name,"destructor ~EquivalenceRelation()"); };
    ...
    bool Implies(const EquivalenceRelation& EquivalenceRelation) const 
    {
     for (const ulong* x = registered; x < next_registered; x++)
       if (!EquivalenceRelation.Equivalent(*x,can_rep[*x])) return false;
     return true;      
    };
