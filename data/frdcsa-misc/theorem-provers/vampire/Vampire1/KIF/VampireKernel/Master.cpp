...
void Master::backSimplify()
{
  ...
  switch (_continuation)
    ...
    case Continuation_Init:
      break;
    case Continuation_InputClause0_BackSimplify:
      // same as below
  ...
  if (_parameters.backward_demodulation > 0)
    {
    next_simplifier:
      if (_queueForBackSimplification.dequeue(currentSimplifier))
	{ 
	  ASSERT(currentSimplifier->flag(Clause::FlagInQueueForBackSimplification));
	  currentSimplifier->clearFlag(Clause::FlagInQueueForBackSimplification);
	  if (backDemodulationQuery(currentSimplifier))
	    { 
		...
		{         
		  ...
		  if (collectBackSimplified())
		    {   
		      currentSimplifier->setFlag(Clause::FlagHasChildren);
...
void Master::inputClause0(Clause* cl)
{
  ...
  switch (_continuation)
    ...
    case Continuation_Init:
      break;
    case Continuation_InputClause0_ProcessCollectedInput:
      cancelTermination();
      goto process_collected_input;
    case Continuation_InputClause0_BackSimplify:
      cancelTermination();
    ...
    default:
      ICP("ICP0");
      RuntimeError::report("Bad _continuation in VK::Master::inputClause0(Clause* cl)");
    };
  ...
  if (_freshClause.LoadClause(cl))
	...
	{  
	  ...
	  _timer.stop();
	  _timerIsOn = false;
	  _continuation = Continuation_InputClause0_ProcessCollectedInput;
	  return; 
	};  
      _continuation = Continuation_Init;
      ...
      if (_terminated)
	{
	  _timer.stop();
	  _timerIsOn = false;
	  _continuation = Continuation_InputClause0_BackSimplify;
	  return; 
	};
      ASSERT(_queueForBackSimplification.empty());
      _continuation = Continuation_Init;
...
void Master::processCollectedInput0()
{
  CALL("processCollectedInput0()");
  Clause* newClause;  
  ...
  switch (_continuation)
    ...
    default:
      ICP("ICP0");
      RuntimeError::report("Bad _continuation in VK::Master::processCollectedInput0()");
    };


  ASSERT_IN(_backSubsumption._termTraversal.checkIntegrity(),"COP1");
...
bool Master::backSubsumeBy(Clause* cl)
{
  ...
  if (_backSubsumption.query(cl))
	...
	{ 
	  _statBackSubsumed++;
	  cl->MakeVIP();
	  subsumedClause = (Clause*)_backSubsumption.subsumed();
...
void Master::reset() 
{
  ...
  options.reset(); 
  resetStatistics();
  _backDemodulation.resetStat(); 
  ASSERT(clean());    
}; //  void Master::reset() 

void Master::supportClausesIntoActive0()
{
  CALL("supportClausesIntoActive0()");
  _passive.ResetIterator();
  for (Clause* cl = _passive.IterNext(); cl; cl = _passive.IterNext())
    {
      if (cl->isSupportClause())
	{
	  _passive.Remove(cl); // safe even with iteration
	  ASSERT(cl->mainSet() == Clause::MainSetPassive);
	  cl->setMainSet(Clause::MainSetFree);
	  makeActive0(cl);
	};
