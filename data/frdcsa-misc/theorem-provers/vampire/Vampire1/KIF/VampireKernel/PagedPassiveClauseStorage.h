//
// File:         PagedPassiveClauseStorage.h
...
#include "GlobAlloc.h"
#include "BitWord.h"
#include "PageQueueDisciplineStorage.h"
#include "Term.h"
...
class PagedPassiveClauseStorage
{
 public: 
	  ...
	  if (_numOfRemainingLiterals)
	    {
	      ...
	      _cursor++;
	      _currentLiteralIsInherentlySelected = (bool)_cursor->Content();
	      _cursor++;
	      return true;
	    }
	  else return false;   
  ...
  class StoredClause
    ...
    public:   
      ...
      const BitWord& rules() const { return _profile.rules();  };
  ...
  class StoredClauseHandler
    ...
    public:
      StoredClauseHandler() {};
      ~StoredClauseHandler() {};
      operator bool() const { return (bool)_memory; };
    private:
      void* _memory;
      friend class PagedPassiveClauseStorage;
    }; // class StoredClauseHandler 
  ...
  class Iterator
	...
	{
	  CALL("Iterator::reset(const PagedPassiveClauseStorage& storage)");
	  _storage = (PagedPassiveClauseStorage*)(&storage);
 ...
 public:
  ...
  void  openClause(StoredClauseHandler& handler,long clNum,long clSize,
                   long clWeight,const BitWord rules,ulong numOfLits,
                   ulong numOfAncestors); 

  bool openClause(long clNum,long clSize,long clWeight,
                  const BitWord rules,ulong numOfLits,ulong numOfAncestors)
    {
      CALL("openClause(long clNum,long clSize,long clWeight,const BitWord rules,ulong numOfLits,ulong numOfAncestors)");
      StoredClauseHandler handler;
      openClause(handler,clNum,clSize,rules,numOfLits,numOfAncestors);
      return (bool)handler;
    }; // bool openClause(long clNum,long clSize,long clWeight,const BitWord rules,ulong numOfLits,ulong numOfAncestors)
 ...
 private: 
  static const ulong PageSize = 2048; // in machine words
  // make shure that PageSize qualifies for large object in the allocator
  typedef PageQueueDisciplineStorage<GlobAlloc,PageSize,PagedPassiveClauseStorage> Storage;
 private:
  ...
  static void readFromMemory(void* memory,ClauseProfile& prof,ClauseLiterals& lits,ClauseAncestors& ancestors);

 private:
  static const ulong MaximalCategoryNumber = VampireKernelConst::MaxClauseSize;
 private:
  // parameters 
