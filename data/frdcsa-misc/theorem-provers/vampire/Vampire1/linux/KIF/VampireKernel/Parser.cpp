			      ...
			      const char* fileName)
{
  ...
  switch (itemSort(item))
	...
	char* fullFileName = (char*)Malloc::allocate(strlen(tptpDir) + strlen(includedFileName) + 2);
	...
	Malloc::deallocate((void*)fullFileName,strlen(tptpDir) + strlen(includedFileName) + 2);
	return result;
      };
...
bool Parser::readLiterals(PR_TERM_LIST* lst,Input::LiteralList*& result)
{
  ...
  if (!lst) { result = (Input::LiteralList*)0; return true; };
...
bool Parser::readTerms(PR_TERM_LIST* terms,Input::TermList*& result)
{
  ...
  if (!terms) { result = (Input::TermList*)0; return true; };
  Input::TermList* tail;
...
bool Parser::readSymbolPrecedence(PR_LITERAL* item)
{
  ...
  long arity = strtol(arg3->Symbol()->Symbol(),(char**)0,10);
  long precedence = strtol(arg4->Symbol()->Symbol(),(char**)0,10);
...
bool Parser::readSymbolWeight(PR_LITERAL* item)
{
  ...
  VampireKernel::TermWeightType weightConstPart = 
    strtol(arg4->Symbol()->Symbol(),(char**)0,10);
...
bool Parser::readAnswerPredicateDeclaration(PR_LITERAL* item)
{
  ...
  long arity = strtol(arg2->Symbol()->Symbol(),(char**)0,10);
