...
long TmpLitList::computeWeightBase() const 
{
  ...
  long res = 0L;
  for (const TMP_LITERAL* el = first(); el; el = el->next())
    {
      res += el->weightBase(); 
    };
  return res;
  ...
  for (const TMP_LITERAL* el = first(); el; el = el->next())
      ...
      else // non-answer literal
	{
	  ...
	  if (el->isEquality()) { stat.equations++; }
		...
		{
		  //if (GlobalHeaderWeighting.Compare(el->header(),stat.max_noneq_header) == Greater)
		  if (Signature::current()->compareHeaderPrecedences(el->header().polarity(),
							    el->header().functor(),
							    stat.max_noneq_header.polarity(),
							    stat.max_noneq_header.functor()) == Greater)
...
void TmpLitList::renameVariables(INT_VAR_RENAMING& varRenaming)
{
  ...
  for (TMP_LITERAL* el = last(); el; el = el->previous()) 
...
bool TmpLitList::weed(bool& eqTautology)
{
  ...
  while (remainingLits.nonempty())
    {
      TMP_LITERAL* lit = remainingLits.pop();
...
void TmpLitList::order(TmpLitList& result,const VAR_WEIGHTING& varWeighting)
{
  ...
  if (numOfAllLiterals() == 2UL)
    { 
      TMP_LITERAL* fst = pop();
      TMP_LITERAL* snd = pop();
      if (fst->compareWithVarWeighting(*snd,varWeighting) == Less) 
	{
	  result.push(fst); 
	  result.push(snd); 
	  return;
  ...
  TMP_LITERAL* pivot = pop();  
  TmpLitList smaller;
  TmpLitList greater; 
  do
    {
      TMP_LITERAL* lit = pop();
      if (lit->compareWithVarWeighting(*pivot,varWeighting) == Less) 
	{ 
	  smaller.push(lit); 
	} 
      else 
