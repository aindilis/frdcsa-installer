 ...
 check_tag:
  ...
  switch (com->Tag())
    ...
    case IU_COMMAND::VAR :
      ...
      if (free_var)
	{
	  if (proc.BindWithOccCheck((Unifier::Variable*)bind,same_var))
	    {
      ...
      else
	{
	  ...
	  if (proc.ExtractUnifiable((PREFIX_SYM*)bind))
	    {
              _backtrackPoints.push(com);
	...
	if (free_var)
	  {
            if (OCCUR_CHECK::Occurs((Unifier::Variable*)bind,com->Term()))
	      {
		goto func_try_alternative;
	      }
            else // assignment possible
	      {
		Unifier::current()->BindAlone((Unifier::Variable*)bind,com->Term());
	  ...
	  {
            if (UNIFICATION::UnifyComplex((PREFIX_SYM*)bind,com->Term()))
	      {
	...
	if (free_var)
	  ...
	  {
            if (UNIFICATION::UnifyPlain((PREFIX_SYM*)bind,com->Term()))
	      {
	...
	if (free_var)
	  ...
	  {
            if (UNIFICATION::MatchGroundPlain((PREFIX_SYM*)bind,com->Term()))
	      {
	...
	if (free_var)
	  ...
	  {
            if (com->Symbol() == ((PREFIX_SYM*)bind)->Head())
	      {
 ...
 rollback:
  ...
  switch (act) 
    ...
    case ALTER_BINDING_WITH_OC_SAME_VAR :
      ...
      var = (Unifier::Variable*)(com->Symbol().var()->Unref());
      ASSERT(Unifier::current()->isVariable((void*)var));
