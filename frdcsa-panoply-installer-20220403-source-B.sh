...
if [ $INSTALL_WITHOUT_NETWORK == true ]; then
    ...
    export WITHOUT_NETWORK_DATADIR="201504014-1/var/lib/myfrdcsa/codebases/interhnal/classify/data/.incoming"
fi
...
if [ ! -d "/etc/myfrdcsa/config" ]; then
	...
	if [ $INSTALL_WITHOUT_NETWORK == true ]; then
	    ...
	    su -c "git clone ssh://andrewdo@10.0.0.220/gitroot/myfrdcsa"
	fi
...
if ! dpkg -l | grep -E 'mysql-server-[0-9]+' | grep -q '^ii'; then
    ...
    apt-get -q -y $USE_FORCE_YES install mysql-server mysql-client
...
if [ $PRIVATE_INSTALL == true ]; then
	...
	if [ $INSTALL_WITHOUT_NETWORK == true ]; then
	    ...
	    su $USER -c "git clone ssh://readonly@posi.frdcsa.org/gitroot/.myconfig-private"
	fi
    else
	if [ $UPDATE_REPOS == true ]; then
...
if [ $PRIVATE_INSTALL == true ]; then
	...
	if [ $INSTALL_WITHOUT_NETWORK == true ]; then
	    ...
	    su $USER -c "git clone ssh://andrewdo@10.0.0.220/gitroot/frdcsa-private"
	fi
...
if [ ! -d "/var/lib/myfrdcsa/codebases/internal/kbfs/data/mysql-backups/mysql-backup" ]; then
	...
	if [ $INSTALL_WITHOUT_NETWORK == true ]; then
	    ...
	    su $USER -c "git clone ssh://readonly@posi.frdcsa.org/gitroot/mysql-private-backup"
	fi
...
if ! perldoc -l File::Signature; then
    cpanm $CPANMADDITIONAL -f File::Signature
fi
export PERL_MM_OPT=$TMP_PERL_MM_OPT
...
if ! perldoc -l Dir::List; then
    cpanm $CPANMADDITIONAL -f Dir::List
fi
...
if ! perldoc -l Text::Quote; then
    cpanm $CPANMADDITIONAL --force Text::Quote
fi
...
if ! perldoc -l Tk::GraphViz; then
    cpanm $CPANMADDITIONAL --force Tk::GraphViz
fi
...
if ! perldoc -l AI::Prolog; then
    cpanm $CPANMADDITIONAL --force AI::Prolog
fi
...
# echo "Starting SPSE2"
# su $USER -c "source $THE_SOURCE && cd /var/lib/myfrdcsa/codebases/minor/spse && XAUTHORITY=/home/$USER/.Xauthority NONINTERACTIVE=true install-script-dependencies \"spse2 -c Org::FRDCSA::Verber::PSEx2::Do -W 10000\""
...
if ! perldoc -l Tk::Month; then
    cpanm $CPANMADDITIONAL -f Tk::Month
fi
export PERL_MM_OPT=$TMP_PERL_MM_OPT
...
if ! perldoc -l Lingua::LinkParser; then
    cpanm $CPANMADDITIONAL -f Lingua::LinkParser
fi
export PERL_MM_OPT=$TMP_PERL_MM_OPT
...
if ! [ -d "/var/lib/myfrdcsa/sandbox/stanford-parser-20110627/stanford-parser-20110627" ]; then
    ...
    su $USER -c "cp -ar $DATA_DIR/frdcsa-misc/var/lib/myfrdcsa/sandbox/stanford-parser-20110627 .y"
fi
if ! [ -d "/var/lib/myfrdcsa/sandbox/stanford-parser-20110627/stanford-parser-20110627" ]; then
    echo "ERROR: Stanford-Ner-20080306 did not copy"
    exit 1
...
cpanm $CPANMADDITIONAL WordNet::Tools WordNet::SenseRelate::AllWords WordNet::QueryData 

# su $USER -c "source $THE_SOURCE && cd /var/lib/myfrdcsa/codebases/minor/nlu/systems/annotation && NONINTERACTIVE=true USE_FORCE_YES=--force-yes install-script-dependencies \"./process-2.pl -W\""

# su $USER -c "source $THE_SOURCE && cd /var/lib/myfrdcsa/codebases/internal/clear && NONINTERACTIVE=true USE_FORCE_YES=--force-yes install-script-dependencies \"cla -r /var/lib/myfrdcsa/codebases/minor/action-planner/OConnor.pdf -W\""
