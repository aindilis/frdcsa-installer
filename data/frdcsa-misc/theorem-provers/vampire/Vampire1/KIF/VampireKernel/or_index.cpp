...
#ifdef NO_DEBUG
  _literalIndex((OR_INDEX_TREE_NODE*)0,(int)0),
#else
  _literalIndex((OR_INDEX_TREE_NODE*)0,"OR_INDEX::_literalIndex"),
#endif

  forward_para_from_var_tree((OR_INDEX_TREE_NODE*)0),
  backward_para_tree((OR_INDEX_TREE_NODE*)0)
{
...
inline void OR_INDEX::Integrate(TERM* lit,Clause* cl,unsigned lit_num)
{
  CALL("Integrate(TERM* lit,Clause* cl,unsigned lit_num)");
  if (lit->arity())
    {
...
}; // void OR_INDEX::Integrate(TERM* lit,Clause* cl,unsigned lit_num)

inline void OR_INDEX::integrateBackSupRedex(TERM* redex,Clause* cl,unsigned lit_num,
					    SimplificationOrdering::StoredConstraint* constr)
{
  CALL("integrateBackSupRedex(TERM* redex,Clause* cl,unsigned lit_num,SimplificationOrdering::StoredConstraint* constr)");
  ASSERT(redex->isComplex());
  OR_INDEX_TREE_NODE** tree = &backward_para_tree;
  OR_INDEX_TREE_NODE* leaf;
  pseudo_literal->Args()->SetFirst(redex);
  l_iter.Reset(pseudo_literal,1UL);
  l_iter.Next();
  leaf = IntegrateArgs(tree);
  leaf->SetTerm(redex);
  leaf->addBackSupTriple(lit_num,cl,redex,constr);  
}; // void OR_INDEX::integrateBackSupRedex(TERM* redex,Clause* cl,unsigned lit_num,SimplificationOrdering::StoredConstraint* constr)

inline void OR_INDEX::integrateForBackwardSuperposition(TERM* lit,Clause* cl,unsigned lit_num)
{
  CALL("integrateForBackwardSuperposition(TERM* lit,Clause* cl,unsigned lit_num)");
  ASSERT(lit->arity());
  TERM::Iterator iter;
  if (lit->IsEquality())
    {
...
}; // void OR_INDEX::integrateForBackwardSuperposition(TERM* lit,Clause* cl,unsigned lit_num)

inline void OR_INDEX::integrateForForwardSuperposition(TERM* lit,Clause* cl,unsigned lit_num)
{
  CALL("integrateForForwardSuperposition(TERM* lit,Clause* cl,unsigned lit_num)");
 ...
 integrate_arg: 
  if (arg->isVariable())
    {
      ...
      leaf->SetTerm(*arg);
      leaf->addForwardSupTriple(lit_num,cl,replacing_term,(SimplificationOrdering::StoredConstraint*)0); 
    }
  ...
  stop = true;
  goto integrate_arg;
}; // void OR_INDEX::integrateForForwardSuperposition(TERM* lit,Clause* cl,unsigned lit_num)
...
inline void OR_INDEX::integratePreorderedForForwardSuperposition(TERM* lit,Clause* cl,unsigned lit_num)
{
  CALL("integratePreorderedForForwardSuperposition(TERM* lit,Clause* cl,unsigned lit_num)");
  ...
  leaf->SetTerm(arg->First());
  leaf->addForwardSupTriple(lit_num,cl,replacing_term,(SimplificationOrdering::StoredConstraint*)0);  
}; // void OR_INDEX::integratePreorderedForForwardSuperposition(TERM* lit,Clause* cl,unsigned lit_num)
...
inline OR_INDEX_TREE_NODE* OR_INDEX::IntegrateNonpropLit(TERM* lit)
{
  CALL("IntegrateNonpropLit(TERM* lit)");
  OR_INDEX_TREE_NODE** tree = Tree(*lit);
  l_iter.Reset(lit,1UL);
  l_iter.Next();
  return IntegrateArgs(tree);
}; // OR_INDEX_TREE_NODE* OR_INDEX::IntegrateNonpropLit(TERM* lit)
...
void OR_INDEX::RemovePropLit(TERM* lit,Clause* cl)
{
  CALL("RemovePropLit(TERM* lit,Clause* cl)");
  ASSERT(!lit->arity());
  OR_INDEX_TREE_NODE** tree = Tree(*lit);
  ASSERT(*tree);
  ASSERT((*tree)->IsLeaf());
  ...
  if (!(*tree)->Clauses()) 
    {
      (*tree)->DeleteLeaf();
      *tree = (OR_INDEX_TREE_NODE*)0;
    };
}; // void OR_INDEX::RemovePropLit(TERM* lit,Clause* cl)
 
OR_INDEX_TREE_NODE* OR_INDEX::LeafAndBranch(TERM* t,OR_INDEX_TREE_NODE** tree,OR_INDEX_TREE_NODE**& branch)
{
  CALL("LeafAndBranch(TERM* t,OR_INDEX_TREE_NODE** tree,OR_INDEX_TREE_NODE**& branch)");
  ASSERT(tree);
  ASSERT(t->arity());
  branch = tree;
  OR_INDEX_TREE_NODE* node = *tree;
  
  if (!node) return (OR_INDEX_TREE_NODE*)0; 
 ...
 check_node:
  ...
  return (OR_INDEX_TREE_NODE*)0;
}; // OR_INDEX_TREE_NODE* OR_INDEX::LeafAndBranch(TERM* t,OR_INDEX_TREE_NODE** tree,OR_INDEX_TREE_NODE**& branch)



void OR_INDEX::Remove(TERM* t,Clause* cl,OR_INDEX_TREE_NODE** tree)
{  
  CALL("Remove(TERM* t,Clause* cl,OR_INDEX_TREE_NODE** tree)");
...
void OR_INDEX::RemoveBPCandidates(TERM* t,Clause* cl,OR_INDEX_TREE_NODE** tree)
{  
  CALL("RemoveBPCandidates(TERM* t,Clause* cl,OR_INDEX_TREE_NODE** tree)");
...
void OR_INDEX::RemoveFromBPI(TERM* lit,Clause* cl)
{
  TERM::Iterator iter;
  if (lit->IsEquality() && lit->IsOrderedEq())
    {
      TERM* arg = lit->Args()->First();
      if (!arg->isVariable())
	{
	  iter.Reset(arg);
	  do
...
void OR_INDEX::RemoveBPRedex(TERM* t,Clause* cl)
{
  CALL("RemoveBPRedex(TERM* t,Clause* cl)");
  pseudo_literal->Args()->SetFirst(t);
  RemoveBPCandidates(pseudo_literal,cl,&backward_para_tree);
}; // void OR_INDEX::RemoveBPRedex(TERM* t,Clause* cl)

void OR_INDEX::RemoveFromFPI(TERM* lit,Clause* cl)
{
  bool stop = false;
  TERM* arg = lit->Args();
 remove_arg:
...
void OR_INDEX::Remove(TERM* lit,Clause* cl)
{
  ...
  for (ulong i = 0; i < _literalIndex.size(); i++)
    {
      ...
      _literalIndex[i] = (OR_INDEX_TREE_NODE*)0;
    };
...
#ifndef NO_DEBUG
bool OR_INDEX::clean() const 
{
  CALL("clean() const");
    
  for (ulong i = 0; i < _literalIndex.size(); i++)
    if (_literalIndex[i]) return false;     
  
  if (forward_para_from_var_tree || backward_para_tree) return false;
  return true;
}; // bool OR_INDEX::clean() const 
#endif
...
ostream& operator<<(ostream& str,const VK::OR_INDEX_TREE_NODE::TAG& tag)
{
  switch (tag)
    {
    case VK::OR_INDEX_TREE_NODE::LEAF: return str << "LEAF";
    case VK::OR_INDEX_TREE_NODE::FUNC: return str << "FUNC";
    case VK::OR_INDEX_TREE_NODE::CONST: return str << "CONST"; 
    case VK::OR_INDEX_TREE_NODE::VAR: return str << "VAR";
    case VK::OR_INDEX_TREE_NODE::VAR_FIRST_OCC: return str << "VAR_FIRST_OCC"; 
 ...
 check_sym:
  ...
  if (var) // free variable
	...
	case OR_INDEX_TREE_NODE::VAR :
	    ...
	    if (free_var)
	      {
		if (OCCUR_CHECK::Occurs((Unifier::Variable*)bind,curr_iter->CurrSubterm()))
		  {
		    // try alternative
		    OR_INDEX_PATH::Alternative();
		    if (OR_INDEX_PATH::CurrNode())
		  ...
		  {
		    Unifier::current()->BindAlone((Unifier::Variable*)bind,curr_iter->CurrSubterm());
		    curr_iter->SkipTerm();
		    OR_INDEX_PATH::Below();
	      ...
	      {
		if (UNIFICATION::UnifyComplex((PREFIX_SYM*)bind,curr_iter->CurrSubterm()))
		  {
		    curr_iter->SkipTerm();
		    OR_INDEX_PATH::Below();
...
bool OR_PROCESSOR::ExtractUnifiable(PREFIX_SYM* word) 
...
ostream& operator<<(ostream& str,const VK::OR_PROCESSOR::SITUATION& sit) 
{
  switch (sit)
    {
    case VK::OR_PROCESSOR::START_OF_EXTRACTION : str << "START_OF_EXTRACTION"; break;
    case VK::OR_PROCESSOR::VAR_SAME_VAR : str << "VAR_SAME_VAR"; break;
    case VK::OR_PROCESSOR::VAR_ANYTHING : str << "VAR_ANYTHING"; break;
    case VK::OR_PROCESSOR::FUNCTOR_FUNCTOR : str << "FUNCTOR_FUNCTOR"; break;
    case VK::OR_PROCESSOR::FUNCTOR_FREE_VAR : str << "FUNCTOR_FREE_VAR"; break;
    case VK::OR_PROCESSOR::FUNCTOR_BOUND_VAR : str << "FUNCTOR_BOUND_VAR"; break;
    };
  return str;
