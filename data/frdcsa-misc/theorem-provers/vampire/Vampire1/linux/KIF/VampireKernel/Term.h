...
class TERM
{
 public:
  ...
  typedef BitSet<GlobAlloc> LitNumMask;
  typedef SmallHeaderSkipList<GlobAlloc,LitNumMask*,ClauseNumber,2,TERM> OccList;
 ...
 public: 
  ...
  bool Negative() const { return (bool)polarity(); };
  bool IsSplitting() const 
    {
      return (content << 17) >> 31;
    };
  ...
  void* operator new(size_t,ulong arity) 
    {    
      ...
      // arity can be 0 
      return GlobAlloc::allocate((arity + 1)*sizeof(TERM) + sizeof(void*));
    };
  void operator delete(void* obj) 
    {
      CALL("operator delete(void* obj)");
      GlobAlloc::deallocate(obj,(((TERM*)obj)->arity() + 1)*sizeof(TERM) + sizeof(void*));
    }; 

  static TERM* AllocLit(const TERM& header)
    {
      TERM* res = (TERM*)GlobAlloc::allocate((header.arity() + 1)*sizeof(TERM) + sizeof(void*));
      res++;
      *res = header;
      return res;
    };  
  void DeleteLit() 
    { 
      GlobAlloc::deallocate(this - 1,(arity() + 1)*sizeof(TERM) + sizeof(void*));
    };
  ...
  Comparison comparePrecedence(const TERM& f) const
    {
      long diff = 
	Signature::current()->symbolPrecedence(functor()) - 
	Signature::current()->symbolPrecedence(f.functor());
      if (diff > 0L) return Greater;
      if (diff < 0L) return Less;
      return Equal;
    };
  ...
  Comparison CompareLex(const TERM* t) const;
  ...
  OccList* Occurences() 
    {
      return (OccList*)((this + arity() + 1)->First()); 
    }; 
  const OccList* Occurences() const
    {
      return (const OccList*)((this + arity() + 1)->First()); 
    }; 
  void InitOccurences() { (this + arity() + 1)->SetFirst((TERM*)0); };

  void DestroyOccurences() 
    {
      CALL("DestroyOccurences()");               
      ASSERT(isComplex());
  ...
  void RegisterOccurence(ulong clauseNum,ulong litNum) 
    {
      CALL("RegisterOccurence(ulong clauseNum,ulong litNum)");
      ASSERT(isComplex());
      OccList* occList = Occurences();
      if (!occList) { occList = new OccList(); (this + arity() + 1)->SetFirst((TERM*)occList); };
      bool newNode;
  ...
  void RemoveOccurences(ulong clauseNum)
    {
      CALL("RemoveOccurences(ulong clauseNum)");
      ASSERT(isComplex());
      OccList* occList = Occurences();
      if (!occList) return;
      LitNumMask* litNumMask;
...
class TERM::Traversal 
{
 ...
 public:
  ...
  Traversal(const char* name) : subterms()
    {
  ...
  Traversal(const TERM& t) : subterms()
    {
  ...
  Traversal(const char* name,const TERM& t) : subterms()
    {
 ...
 private:
  TERM wrapper[2];
  State currState;
  Stack<const TERM*,VampireKernelConst::MaxTermSize> subterms;
  const TERM* trm;
  const TERM* sym;
#ifndef NO_DEBUG
...
{
 private:
  Stack<TERM*,VampireKernelConst::MaxTermSize> subterms;   
  TERM* sym;
  bool isVar;  
 public:
  Iterator() : subterms() {};   
  ...
  TERM& symbolRef() { return *sym; };
  const TERM& symbolRef() const { return *sym; };
  TERM* CurrPos() const { return sym; };
...
class BracketCounter
{
 ...
 public:
  ...
  bool IsOpen() const { return curr_holes; };
...
class ExpandingBracketCounter
{
 public:
  ...
  bool isOpen() const { return (bool)_currentNumOfHoles; };
 ...
 private: 
  ExpandingStack<GlobAlloc,long,128UL,ExpandingBracketCounter> _allHoles; 
...
: private TERM::Iterator,
	    private BracketCounter<VampireKernelConst::MaxTermDepth>,
	    private DepthCounter 
{
 public:
  IteratorWithDepth() {};
  void Reset(TERM* const t,ulong d) 
    {
      CALL("Reset(TERM* const t,ulong d)");
      ASSERT(d);
      Iterator::Reset(t);
  ...
  TERM& symbolRef() { return Iterator::symbolRef(); };
...
{
 public:  
  SmartDepthCounter() : BracketCounter<MaxDepth>(), DepthCounter() {};
...
class TERM::OpenCompact
{
 ...
 private:
  Stack<TERM*,VampireKernelConst::MaxTermDepth> _holes;
  Stack<ulong,VampireKernelConst::MaxTermDepth> _numOfHoles;
  TERM* _currHole;
...
{
 public:
  OpenUnshared() : _depthCounter(), _holes(), _freeHoles(), _allocated()
    { 
 ...
 private: 
  ...
  Stack<TERM*,VampireKernelConst::MaxTermDepth> _holes;
  Stack<ulong,VampireKernelConst::MaxTermDepth> _freeHoles;
  TERM* _allocatedLit;
  Stack<TERM*,VampireKernelConst::MaxTermSize> _allocated;
}; // class TERM::OpenUnshared
...
inline Comparison TERM::CompareLex(const TERM* t) const
{
  ...
  do
	...
	{ 
	  if (iter1.CurrentSymIsVar() || iter2.CurrentSymIsVar()) return Incomparable;    
...
class TERM::NonvariableWithInfo
{
 public:   
  ...
  template <typename Info> 
    ...
    {
      return (TERM*)(((Info*)GlobAlloc::allocate(sizeWithInfo<Info>(arity))) + 1);
    };
  template <typename Info> 
    static void deleteTopLevel(TERM* trm)
    {
      GlobAlloc::deallocate(infoAddr<Info>(trm),sizeWithInfo<Info>(trm->arity()));
    }; 
}; // class TERM::NonvariableWithInfo
