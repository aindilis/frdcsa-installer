...
class ClassDesc
{
 ...
 public: 
  ClassDesc(const char* nm) 
   : _name(nm), 
     _registeredObjects((SizeAllocDeallocList*)0),
     _minSize(0UL),
     _maxSize(ULONG_MAX)
  {
  ...
  static ostream& outputAll(ostream& str,bool objectStatistics); 
 private:
  const char* _name;
...
static BK::ClassDesc::InitStatic classDescInitStatic;
