...
class DTREE_NODE
 {
  private:
   ...
   DTREE_NODE() {};
  public:
   DTREE_NODE(TERM* p) : _tag(p), _next((DTREE_NODE*)0) {}; // for making leaves
   DTREE_NODE(const TERM& sym)  // for intermediate nodes
    : _tag(sym),
      _less((DTREE_NODE*)0), 
      _greater((DTREE_NODE*)0)
    {};
   ~DTREE_NODE() {};
   ...
   inline void* operator new(size_t)
    {
     return GlobAlloc::allocate(sizeof(DTREE_NODE));
    };
   inline void operator delete(void* obj)
    {
     GlobAlloc::deallocate(obj,sizeof(DTREE_NODE));
    };
...
inline void DTREE::InfoForDeletion(TERM::Iterator& trm,
                                   ...
                                   DTREE_NODE*& last_fork)
 {
  ...
  track_sym:
   ...
   ASSERT((*branch_to_destroy)->IsLeaf() ? (bool)last_fork : true);
