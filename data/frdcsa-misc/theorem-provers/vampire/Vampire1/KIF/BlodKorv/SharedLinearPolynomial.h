...
namespace BK 
{
  ...
  template <class Alloc,class CoeffType,ulong MaxSize>
    ...
    public:
      SharedLinearPolynomial(const Member& m) : _member(m), _refCounter(0L), _tail((SharedLinearPolynomial*)0) 
	{
      ...
      ~SharedLinearPolynomial() 
	{
	  ...
	  if (_tail) _tail->decReferenceCounter();
	};
...
namespace BK 
{
  template <class Alloc,class CoeffType,ulong MaxSharedLinearPolynomialSize>
	...
	{
	  CALL("integrate()");
	  if (!_members) return (SharedLinearPolynomial<Alloc,CoeffType,MaxSharedLinearPolynomialSize>*)0;
