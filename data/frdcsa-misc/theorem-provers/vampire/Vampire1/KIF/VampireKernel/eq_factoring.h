...
class EQ_FACTOR
 {
  ...
  public:
   ...
   void LoadST(PREFIX_SYM* eq_lit) 
    {
     CALL("LoadST(PREFIX_SYM* eq_lit)");
     ASSERT(PREFIX::CorrectComplexTerm(eq_lit));
     st_unordered = eq_lit->Head().Func().IsUnorderedEq();
     s = eq_lit + 1;
     t = s->Bracket();
    };
   void LoadS_T_(PREFIX_SYM* eq_lit) 
    {
     CALL("LoadS_T_(PREFIX_SYM* eq_lit)");
     ASSERT(PREFIX::CorrectComplexTerm(eq_lit));
     s_t_ = eq_lit;
     s_ = eq_lit + 1;
     t_ = s_->Bracket();
...
class EQ_FACTORING
 {
  private:
   ...
   PREFIX_SYM pos_eq[VampireKernelConst::MaxClauseSize]; // memory

   PREFIX_SYM* _freePosEqMemory;

   ExpandingStack<GlobAlloc,PREFIX_SYM*,32UL,EQ_FACTORING> _positiveEqualities;

   long last_pos_eq;
   long last_max_pos_eq;
   
   ExpandingStack<GlobAlloc,TERM*,32UL,EQ_FACTORING> _otherLiterals;
     ...
     if (lit->Positive() && lit->IsEquality())
      {
       ...
       //PREFIX_SYM* mem = PosEq(last_pos_eq);
       _positiveEqualities.push(_freePosEqMemory);
       DOP(PREFIX_SYM* debugMem = _freePosEqMemory);
