...
void OLD_FS_CODE::destroy(void (*destroyIndexedObj)(void*))  
{
...
void OLD_FS_CODE::integrate(TmpLitList& component,void* name)
{
  ...
  for (TMP_LITERAL* el = component.first(); el; el = el->next())
    integrateLit(el);
...
void OLD_FS_CODE::integrateLit(TMP_LITERAL* lit)
{
  ...
  integrator.endOfLit();  
}; //OLD_FS_CODE::integrateLit(TMP_LITERAL* lit)  
...
{
  FS_COMMAND* newInstr = FS_COMMAND::NewSUCCESS((Clause*)name);
  newInstr->SetFork(*node_addr);
  *node_addr = newInstr;
