...
// pretty-printing in the TPTP format
void Clause::prettyPrint ( int indent, ostream& str ) const
{
  // initial indent
  ...
  Literal::List* lits = _literals;  
  Literal::List* next = _literals->tail ();
...
// pretty-printing in the TPTP format
ostream& operator << ( ostream& str, const Clause* c ) 
{
  ...
  Literal::List* lits = c->literals ();
  Literal::List* next = c->literals()->tail ();
...
Term* Clause::isFunctionDefinition ( Term*& r ) const
{
  if ( _literals->isEmpty () ) // empty clause
    return 0;
  if ( _literals->tail()->isNonEmpty () ) // more than one literal
    return 0;

  // now the clause has one literal
  Literal* l = _literals->head();
  
  if ( l->negative() ) // negative literal
    return 0;

  return l->atom()->isDefinition (r);
...
void Clause::normalize ()
{
  ...
  Literal::List::Iterator lits ( literals() );
  while ( lits.more() ) {
    Literal* l = lits.next ();
    l->normalize ();
    srt.add ( l );
  ...
  Literal::List* sorted = Literal::List::empty ();
  // sort is now sorted, create the resulting sorted list of literals
  for (int i = length-1; i >=0; i--) {
    Literal::List::push ( srt[i], sorted );
  }

  // delete the literal list, if necessary
  literals (sorted);
} // Clause::normalize


// used to compare clauses. The clause literals must be normalised before
bool Clause::isless (const Clause* cls2) const
{
  Literal::List* lits1 = literals ();
  Literal::List* lits2 = cls2->literals (); 
