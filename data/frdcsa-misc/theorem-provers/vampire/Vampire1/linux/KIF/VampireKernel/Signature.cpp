...
namespace VK
{
long Signature::InitStatic::_count; // Implicitely initialised to 0L
...
#ifdef NO_DEBUG
    _symbolName((char*)0,(int)0),
#else
    _symbolName((char*)0,"Signature::_symbolName"),
#endif
    _numericConstantValue(DOP("Signature::_numericConstantValue")),
#ifdef NO_DEBUG
    _isNumericConstant(false,(int)0),
    _isInputSymbol(false,(int)0),
    _isPredicate(false,(int)0),
    _isCommutative(false,(int)0),
    _arity(ULONG_MAX,(int)0),
    _isAnswerPredicate(false,(int)0),
#else
...
void Signature::endOfInput()
{
  ...
  for (ulong fn = 0; fn < nextSymbolNumber(); fn++)
    {
