...
class Clause;
class Flatterm;
typedef EquivalenceRelation<VampireKernelConst::MaxClauseSize> FS_EQ_CONSTR; 
...
{  
 private:
  ...
  MultisetOfVariables<VampireKernelConst::MaxNumOfVariables> vars; 
 ...
 public:
  FS_CODE_INTEGRATOR(const Flatterm** subst) 
    ...
    { 
      ...
      sig_to_insert = (FunSet*)0;
      sig_to_shift = (FunSet*)0;
    };
  ~FS_CODE_INTEGRATOR() { CALL("destructor ~FS_CODE_INTEGRATOR()"); };
...
class FS_CODE_REMOVAL
{
 ...
 public:
  ...
  ~FS_CODE_REMOVAL() { CALL("destructor ~FS_CODE_REMOVAL()"); };
  void Reset(FS_COMMAND** n,Clause* cl) 
    {
      removed_clause = cl;
      node_addr = n; 
...
class FS_CODE 
{
 public:
  ...
  ~FS_CODE() { CALL("destructor ~FS_CODE()"); };
#ifndef NO_DEBUG      
  bool clean() const
    {
      return oldStyleCode.clean() && (!tree);  
    };
#endif
  ...
  void Destroy()
    {
      if (oldStyle) { oldStyleCode.destroy(); return; };
      if (tree) FS_COMMAND::DestroyTree(tree);
      tree = (FS_COMMAND*)0;
    };
  void Destroy(void (*destroyIndexedObj)(void*))
    {
      if (oldStyle) { oldStyleCode.destroy(destroyIndexedObj); return; };
      if (tree) FS_COMMAND::DestroyTree(tree,destroyIndexedObj);
      tree = (FS_COMMAND*)0;
    };
