...
#if ((defined INTERCEPT_FORWARD_MATCHING_INDEXING) || (defined INTERCEPT_BACKWARD_SUBSUMPTION_INDEXING)) 
#include "Logging.h"
...
int main(int numarg,char** args)
{
  ...
  *jobTime = time((time_t*)0);
  ...
  const char* problemFileName = (char*)(CL.nonoptVal("<problem>"));
  const char* problemFileBaseName = basename(problemFileName);
  ...
  cout << "%=================================================================\n";
  cout << "% Vampire kernel v" << VampireKernel::versionNumber()  
       << " " << VampireKernel::versionDescription() << '\n';  
#ifndef NO_DEBUG 
  cout << "% WARNING: COMPILED WITHOUT -DNO_DEBUG!\n";
  cout << "%          THIS MAY AFFECT THE PERFOMANCE BADLY!\n";         
#endif
#ifdef INTERCEPT_FORWARD_MATCHING_INDEXING
  cout << "% WARNING: FORWARD MATCHING INDEX OPERATIONS ARE LOGGED!\n"; 
  cout << "%          THIS MAY AFFECT THE PERFOMANCE BADLY!\n";         
#endif
#ifdef INTERCEPT_BACKWARD_SUBSUMPTION_INDEXING
  cout << "% WARNING: BACKWARD SUBSUMPTION INDEX OPERATIONS ARE LOGGED!\n"; 
  cout << "%          THIS MAY AFFECT THE PERFOMANCE BADLY!\n";         
#endif
#ifdef INTERCEPT_BACKWARD_MATCHING_INDEXING
  cout << "% WARNING: BACKWARD MATCHING INDEX OPERATIONS ARE LOGGED!\n"; 
  cout << "%          THIS MAY AFFECT THE PERFOMANCE BADLY!\n";         
#endif
  cout << "% Started: " << asctime(localtime(jobTime));
  cout << "% Host:    " << hostname << '\n'; 
  cout << "% Problem: " << problemFileName << '\n';
  cout << "% Command: "; CL.printCommand(cout) << '\n';
  cout << "%=================================================================\n";
  ...
  **********/
  ...
  try 
	   ...
	   symbol = symbol->next())
	{
	  ...
	  switch (symbol->type())
	      ...
	      if (symbol->precedenceAssigned())
		{
	      ...
	      for (const Input::InterpretationList* interpr =  symbol->interpretations();
		   interpr;
		   interpr = interpr->tl())
		{
	   ...
	   cl = cl->tl())
	{
	  ...
	  kernel.beginInputClause(clauseNumber,(void*)cl->hd(),isGoal,isSupport);
	  ++clauseNumber;
 ...
 normal_kernel_termination:
  ...
  switch (kernel.terminationReason())
      ...
      if (tabulation)
	{
      ...
      cout << "% " << numberOfFoundAnswers << " answer";
      if (numberOfFoundAnswers > 1L) cout << "s";
      cout << " found so far.\n";
      break;
  ...
  DF; GlobAlloc::outputPersistent(cout,10000UL);
...
void communicateTerm(const Input::Term* term,VampireKernel& kernel)
{
  ...
  switch (term->top()->type())
    ...
    case Input::Symbol::Function:
      ...
      for (const Input::TermList* args = term->arguments();
	   args;
	   args = args->tl())
	{
	  communicateTerm(args->hd(),kernel);
