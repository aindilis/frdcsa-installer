...
class Array
{
 public:
  ...
  Array& operator=(const Array& a)
  {
   ...
   if (_currSize)
    {
     _memory = (ElType*)Alloc::allocate(sizeof(ElType)*_currSize);
  ...
  {
   CALL("init(const ElType& initElemVal,int /* dummy */)");
  ...
  {
   CALLM(_name,"destroy()"); 
   if (_currSize) Alloc::deallocate((void*)_memory,sizeof(ElType)*_currSize);
   init();
  };
  ...
  void expand(ulong i)
  {
   ...
   ASSERT(i < newSize);
   ElType* newMemory = (ElType*)Alloc::allocate(sizeof(ElType)*newSize);
   if (_currSize)
    {
  ...
  ulong size() const { return _currSize; };
   ...
   bool checkObj()
   {
     ...
     if (_currSize)
      {
       ObjDesc* objDesc = Alloc::hostObj((void*)_memory);
