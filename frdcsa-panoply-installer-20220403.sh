...
if [ $PRIVATE_INSTALL == true ]; then
    export SOURCE_HOST="ai2.frdcsa.org"
fi
...
if ! grep -q 'FRDCSA Xresources' /home/$USER/.Xresources; then
    cat $DATA_DIR/helper/Xresources >> /home/$USER/.Xresources
    # xrdb -merge .Xresources
fi

if ! grep -q 'FRDCSA screenrc' /home/$USER/.screenrc; then
    cat $DATA_DIR/helper/screenrc >> /home/$USER/.screenrc
...
if [ ! -d "/etc/myfrdcsa/config" ]; then
	...
	if [ $INSTALL_WITHOUT_NETWORK == true ]; then
	    ...
	    su -c "git clone ssh://andrewdo@$SOURCE_HOST/etc/myfrdcsa"
	fi
