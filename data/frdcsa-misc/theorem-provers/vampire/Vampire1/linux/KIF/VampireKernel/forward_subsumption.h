...
class FS_QUERY
{
 ...
 public:
  ...
  ~FS_QUERY() { CALL("destructor ~FS_QUERY()"); };
  void ResetQuery()
    {
      CALL("ResetQuery()");
 ...
 private: // structure  
  ...
  Array<GlobAlloc,LIT*,128UL,FS_QUERY> _litsWithHeader;
  const ulong _posOrdEqHeaderNum;
  const ulong _negOrdEqHeaderNum;
  
  ExpandingStack<GlobAlloc,ulong,32UL,FS_QUERY> _presentHeaders;
...
class FS_STATISTICS
{
 public:
  FS_STATISTICS() 
    ...
    compInstr(0UL) 
    {   
      CALL("constructor FS_STATISTICS()");
    };
  ~FS_STATISTICS() {};
  void anotherCall() { calls++; };
  void anotherInstr() { totInstr++; };
  void anotherCompInstr() { compInstr++; };
  void anotherCompSucc() { compSucc++; };
  unsigned long statCalls() const { return calls; };
  unsigned long statTotInstr() const { return totInstr; };
  unsigned long statNonCompInstr() const { return totInstr - compInstr; };
  unsigned long statCompInstr() const { return compInstr; };
  unsigned long statCompSucc() const { return compSucc; };
  unsigned long statCompFail() const { return compInstr - compSucc; };  
 private: 
  unsigned long calls;
  unsigned long totInstr;
  unsigned long compInstr;
  unsigned long compSucc;
};
#endif
}; // namespace VK
...

{
 public:
  ...
  ~ForwardSubsumption() {};
  void Destroy() { FS_CODE::Destroy(); };
#ifndef NO_DEBUG
  bool clean() const { return FS_CODE::clean(); };
#endif
  Clause* Subsume() 
    {
      CALL("Subsume()");
 ...
 private:
  ...
  ExpandingStack<GlobAlloc,LIT*,32UL,ForwardSubsumption> backtrack_lit;
