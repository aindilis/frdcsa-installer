...
class LARGE_WEIGHT_FORMULA
 {
  ...
  public:
   ...
   inline void CompileSimple(long coefficient,PREFIX_SYM* t) // without substitution
    {
     ...
     while (!iter.Empty());
    }; // void CompileSimple(long coefficient,PREFIX_SYM* t)

 
   inline void Compile(long coefficient,PREFIX_SYM* t)
    {
     if (t->Head().isVariable()) 
      {
       bool free_var;
       ...
       if (free_var)
        {
         RegisterVar(coefficient,(const Unifier::Variable*)bind);
         return;
        }
       else { t = (PREFIX_SYM*)bind; }; 
      };
     ...
     while (!iter.Empty());
    }; // void Compile(long coefficient,PREFIX_SYM* t)


   inline void CompileSubst(long coefficient,const Unifier::Variable* var)
    {
     bool free_var;
     void* bind = var->Unref(free_var);
     if (free_var)
      {
       RegisterVar(coefficient,(const Unifier::Variable*)bind);
      }
     else Compile(coefficient,(PREFIX_SYM*)bind);
    };   
   ...
   inline void Substitute(LARGE_WEIGHT_FORMULA& result)
    {
     ...
     for (Unifier::Variable* var = var_counter.NextRegistered(); var; var = var_counter.NextRegistered())
      {
...
class ORD_FORMULA
 {
  ...
  public:
   ...
   inline void Compile(PREFIX_SYM* t1,PREFIX_SYM* t2)
    {
       ...
       if (iter1.CurrVar())
        ...
        if (iter2.CurrVar())
        ...
        else // both are functors
         {
          ...
          if (func1 != func2) return (func1.comparePrecedence(func2) == Less); 
         }; 
       iter1.SkipSym();
       iter2.SkipSym();
      }      
       ...
       if (iter1.CurrVar())
        ...
        if (iter2.CurrVar())
        ...
        else // both are functors
         {
          func1 = iter1.CurrSubterm()->Head().Func();     
          func2 = iter2.CurrSubterm()->Head().Func(); 
          if (func1 != func2) return (func1.comparePrecedence(func2) == Greater); 
         }; 
       iter1.SkipSym();
       iter2.SkipSym();
      }      
...
class WEIGHT_FORMULA
 {
  ...
  public:
     ...
     for (const Unifier::Variable* var = var_counter.NextRegistered();var;var = var_counter.NextRegistered())
      {
       bool free_var;
       void* bind = var->Unref(free_var);
       long w = (free_var)? 1 : PREFIX::Weight((PREFIX_SYM*)bind); 
   ...
   inline void Compile(PREFIX_SYM* t)
    {
     CALL("Compile(PREFIX_SYM* t)");
     if (t->Head().isVariable()) 
      {
